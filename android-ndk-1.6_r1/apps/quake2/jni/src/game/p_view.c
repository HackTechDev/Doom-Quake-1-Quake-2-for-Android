...
// debut mod : qmenu
	...
	if ( ((ent->client->showscores  || ent->client->pers.scanner_active) && deathmatch->value && !(level.framenum & SCANNER_UPDATE_FREQ) &&
!(ent->client->showmenu) && !(ent->client->showmsg) )  || (ent->client->pers.scanner_active & 2))
